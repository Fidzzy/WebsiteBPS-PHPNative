...
<body>
  <header>
    ...
    <nav>
      <a href="../publikasi/page09A.php">Publikasi</a>
  ...
  <main>
    ...
    <?php if (!empty($_GET['status'])): ?>
      <?php if ($_GET['status'] === 'gagal'): ?>
        <p id="pesanErrorDb" style="display:block; width:60%; margin:0 auto 15px auto; padding:12px 16px; background-color:#fff3f3; border:1px solid #e74c3c; border-radius:4px; color:#c0392b; font-size:16px;">
          <?= htmlspecialchars($_GET['pesan'] ?? 'Terjadi kesalahan.') ?>
        </p>
      <?php endif; ?>
    <?php endif; ?>

    <p id="pesanError" style="display:none; width:60%; margin:0 auto 15px auto; padding:12px 16px; background-color:#fff3f3; border:1px solid #e74c3c; border-radius:4px; color:#c0392b; font-size:16px;"></p>
